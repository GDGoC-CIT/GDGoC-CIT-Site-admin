// admin/src/pages/EditMember.jsx
...
export default function EditMember() {
  ...
  const batchOptions = useMemo(() => {
    ...
    const startYear = 2019; // Starting from 2019
    const arr = [];

    // Generate batches from 2019 to current year + 1
    for (let year = startYear; year <= currentYear + 1; year++) {
      arr.push(`${year}-${year + 1}`);
    }

    return arr.sort((a, b) => parseInt(b) - parseInt(a)); // Sort descending (newest first)
  }, []);
  ...
  useEffect(() => {
    API.get("/courses")
      .then((res) => {
        // Filter out "Computing Department" case-insensitively
  ...
  // Fetch member data
  useEffect(() => {
    API.get("/admin/members/all").then((res) => {
      const m = res.data.find((it) => it._id === id);
      if (!m) return setMsg("Member not found");

      const cleanedDigits = m.phone
        .replace("+91", "")
        .replace(/\s/g, "")
        .trim();
      const formatted = `${cleanedDigits.slice(0, 5)} ${cleanedDigits.slice(
        5
      )}`;

      setName(m.name || "");
      setCourse(m.course?._id || "");
      setYear(m.year || "");
      setPosition(m.position?._id || "");
      setRollNo(m.rollNo || "");
      setGender(m.gender || "");
  ...
  const saveChanges = async (e) => {
    ...
    try {
      await API.put(`/admin/members/${id}`, fd, {
        onUploadProgress: (p) =>
      ...
      <div className="space-y-4 mb-6 sm:mb-8">
        ...
        <div className="sm:hidden space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-linear-to-r from-amber-500 to-orange-600 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
            ...
            <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {adminRole === "superadmin" ? (
                        ...
                        <div className="relative">
                          <select
                            className="w-full bg-slate-900/50 border border-slate-700 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-white text-sm sm:text-base cursor-not-allowed opacity-70"
                            value={batch}
                            disabled
                          >
                            <option
                          ...
                          <div className="absolute inset-0 bg-slate-900/20 rounded-xl cursor-not-allowed"></div>
                        </div>
                      )}
                    </div>
              ...
              <div>
                ...
                <div
                  className={`border-2 border-dashed rounded-2xl p-4 sm:p-6 lg:p-8 text-center cursor-pointer transition-all duration-200 hover:border-indigo-500 hover:bg-slate-900/30 group ${
          ...
          <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-700/50">
            <button
              ...
              className={`w-full font-semibold py-3 sm:py-4 px-4 sm:px-6 rounded-xl transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg ${
                formChanged
                  ? "bg-linear-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white focus:ring-amber-500"
